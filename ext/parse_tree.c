...
/* Modified from dump.c
   FIXME: dump.c creates a string but we want to create an S expression.
*/
...
#define D_NULL_NODE wrap_into_node("(null node)", Qnil);
#define D_NODE_HEADER(node) A_NODE_HEADER(node); A("\n");

#define COMPOUND_FIELD(name, name2, block)	\
    do { \
	A_FIELD_HEADER(name); A("\n"); \
	block; \
    } while (0)

#define SIMPLE_FIELD(name, name2, block) \
    do { \
	A_FIELD_HEADER(name); A(" "); block; A("\n"); \
    } while (0)
...
#define F_NODE(name, ann) \
    COMPOUND_FIELD(#name, #name " (" ann ")", \
		   add_to_parse_tree(self, ary, node->name))
...
static VALUE 
parse_tree_for_meth(VALUE self, VALUE _klass, VALUE _method, 
		    VALUE _is_cls_meth) 
{
    VALUE klass = (_klass);
...
#else
	result = rb_parser_dump_tree(node->nd_body, 0);
#endif
    } else {
...
static VALUE 
parse_tree_for_str(VALUE self, VALUE _source, VALUE _filename, VALUE _line) 
{
